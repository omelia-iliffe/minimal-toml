    ...
    fn expect_eol_or_eof(&mut self) -> Result<()> {
        match self.next() {
            Ok(token) => match token.token {
                ...
                t => Err(Error::new(
                    token.range,
                    ErrorKind::UnexpectedToken(t, Expected::EolOrEof),
                )),
            },
            Err(_eof) => Ok(()),
        }
...
impl<'de, 'a> MapAccess<'de> for KeyValuePairs<'a, 'de> {
        ...
        V: DeserializeSeed<'de>,
    {
        ...
        let allow_consume_whitespace = expect_next_peeked! {
            ...
            Token::Equals => {
                // Found = not consuming whitespace after de
                //Consume equals
                let _ = de_self.next().expect("peeked token");
                false
            },
            Token::DoubleSquareBracket(Open) => {
                // Found [[ not consuming whitespace after de
            ...
            Token::BareString => {
                // Found bare string not consuming whitespace after de
                true
            }
        };
